...
// lemma MarginLipBoundDerivation(m: Matrix, m': Matrix, p: nat, q: nat, r: real)
//   requires p < |m|
//   requires q < |m|
//   requires m' == [Minus(m[q], m[p])]
//   requires P1: IsSpecNormUpperBound(r, m')
//   ensures IsMarginLipBound([m], r, p, q)
// {
//   forall v, u: Vector | IsInput(v, [m]) && IsInput(u, [m])
//     ensures Abs((NN([m], v)[q] - NN([m], v)[p]) - (NN([m], u)[q] - NN([m], u)[p])) <= r * Distance(v, u)
//   {
//     assert Q1: _ by {
//       reveal P1;
//       SpecNormIsMvLipBound(m', v, u, r);
//       assert Distance(MV(m', v), MV(m', u)) <= r * Distance(v, u);
//       // distance is absolute difference for singletons
//       assert Abs(MV(m', v)[0] - MV(m', u)[0]) <= r * Distance(v, u);
//       calc {
//         MV(m', v)[0];
//         ==
//         Dot([Minus(m[q], m[p])], v);
...
//         Dot(m[q], v) - Dot(m[p], v);
//       }
//       assert Abs((Dot(m[q], v) - Dot(m[p], v)) - (Dot(m[q], u) - Dot(m[p], u))) <= r * Distance(v, u);
//       assert Abs(Relu(Dot(m[q], v) - Dot(m[p], v)) - Relu(Dot(m[q], u) - Dot(m[p], u))) <= r * Distance(v, u);


//       assert Abs(Dot(Minus(m[q], m[p]), v) - Dot(Minus(m[q], m[p]), u)) <= r * Distance(v, u);
//       assert Abs(Dot(m[q], v) - Dot(m[p], v) - (Dot(m[q], u) - Dot(m[p], u))) <= r * Distance(v, u);
//       assert Abs(Dot(m[q], v) - Dot(m[p], v) - (Dot(m[q], u) - Dot(m[p], u))) <= r * Distance(v, u);
...
//       // distance is absolute difference for singletons
//       assert Abs(Layer(m', v)[0] - Layer(m', u)[0]) <= r * Distance(v, u);
//       // show Abs(Layer(m', v)[0]) >= Abs(NN([m], v)[q] - NN([m], v)[p]) ?
//       calc {
//         Layer(m', v)[0];
//         ==
//         ApplyRelu(MV(m', v))[0];
//         ==
//         ApplyRelu([Dot(Minus(m[q], m[p]), v)])[0];
//         ==
//         Relu(Dot(Minus(m[q], m[p]), v));
//         ==
//         calc {
//           Dot(Minus(m[q], m[p]), v);
//           ==
